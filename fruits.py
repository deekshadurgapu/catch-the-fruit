...
class ScoreBoard():
    ...
    def initGUI(self):
        ...
        Label(self.parent, text="Lives:", font=("Georgia",20, "bold")).grid(row=1, column=2, padx=35, pady=100, sticky=N+W)        
        Label(self.parent, textvariable=self.livesVar, font=("Georgia",20, "bold")).grid(row=1, column=2, padx=60, pady=150, sticky=N+W)        
        
        # Score
        self.scoreVar = IntVar()
        Label(self.parent, text="Score:", font=("Georgia",20, "bold")).grid(row=1, column=2, padx=35, pady=250, sticky=N+W)
        Label(self.parent, textvariable=self.scoreVar, font=("Georgia",20, "bold")).grid(row=1, column=2, padx=50, pady=300, sticky=N+W)        
        
        # High score
        self.highScoreVar = IntVar()
        Label(self.parent, text="Highest Score:", font=("Georgia",20, "bold")).grid(row=1, column=2, padx=0, pady=400, sticky=N+W)
        Label(self.parent, textvariable=self.highScoreVar, font=("Georgia",20, "bold")).grid(row=1, column=2, padx=50, pady=450, sticky=N+W)
    ...
    def gameOver(self):
        self.saveScore()
        tkinter.messagebox.showinfo("CATCH THE FRUITS", "YOUR LIVES ARE OVER")        
        if tkinter.messagebox.askyesno("CATCH THE FRUITS", "PLAY AGAIN??"):
            self.reset()
        else:
            exit()
...
class TheGame(ItemsFallingFromSky,ScoreBoard):
    ...
    def __init__(self,parent):
        ...
        self.parent.title("CATCH THE FRUITS")

        # canvas window
        self.canvas = Canvas(self.parent, width=800, height=600)
        self.canvas.config(background="#FFB6C1")
        ...
        # player character
        self.playerPhoto = tkinter.PhotoImage(file = "images/{}" .format( "saakshi.gif" ) )
